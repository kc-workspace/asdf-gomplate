...
__asdf_bin() {
  ...
  if kc_asdf_is_ref; then
    url="${ASDF_OVERRIDE_REF_REPO:-https://github.com/hairyhenderson/gomplate.git}"
    ...
    url="https://github.com/hairyhenderson/gomplate/releases/download/v{version}/gomplate_{os}-{arch}"
    url="$(kc_asdf_template "$url" "${vars[@]}")"
  ...
  if [[ "$mode" == "custom" ]]; then
    ...
    if kc_asdf_enabled_feature checksum; then
      local checksum_url
      checksum_url="https://github.com/hairyhenderson/gomplate/releases/download/v{version}/checksums-v{version}_sha256.txt"
